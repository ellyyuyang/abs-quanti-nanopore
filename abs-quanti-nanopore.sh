...
# Written by YANG, Yu, last update on Nov 17 2020.
...
# nucleotide ARG database and the structure file (source: https://github.com/xiaole99/ARGs-OAP-v2.0-development): nucleotide-ARG-DB.fasta
...
do seqtk seq -a ${file} > ${file}.fasta;
	...
	#filter out DCS reads by blastn with min 85% similarity, min 90% subLcounts, and max 3600bp length (with this cutoff the final % DCS bases is arounf 1% with is the dosage percentage, 10ng in 1ug)
...
do minimap2  -cx map-ont nucleotide-ARG-DB.fasta ${file} > ${file}_minimap2_modifiednanoARG_algn.paf; #path to the nucleotide ARG database can be replaced here
	...
	join -t$'\t' -1 3 -2 1 -o 1.1 1.2 1.4 1.5 1.3 2.2 2.3 ${file}_minimap2_modifiednanoARG_algn.paf_similarity_subLcounts_primary_S80subL200_seqID_length_ARGaccession_subLcounts /media/YY/databases/nanopore_arg/modified_structure|sed 's/rifampin/rifamycin/g'|uniq > ${file}_minimap2_modifiednanoARG_algn.paf_similarity_subLcounts_primary_S80subL200_seqID_length_subLcounts_ARGaccession_ARGannotation; #path to the modified nucleotide ARG database can be pre-defined
